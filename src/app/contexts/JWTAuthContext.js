...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    const response = await axios.post(
      'https://' + process.env.REACT_APP_BACKEND_URL + 'authentication/login',
      {
        Email: email,
        Password: password,
      }
    ...
    if (status) {
      ...
      if (Position == 0) {
        ...
        const result2 = await axios.post(
          'https://' + process.env.REACT_APP_BACKEND_URL + 'teacher/info',
          {},
          {
            headers: headers,
          }
        ...
        const result2 = await axios.post(
          'https://' + process.env.REACT_APP_BACKEND_URL + 'admin/info',
          {},
          {
            headers: headers,
          }
  ...
  const register = async (P, data) => {
    ...
    if (P == 0) {
      const response = await axios.post(
        'https://' + process.env.REACT_APP_BACKEND_URL + 'authentication/register',
        {
          Email: data.Email,
          Position: 0,
