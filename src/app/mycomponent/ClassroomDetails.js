...
const ClassroomDetails = (props) => {
  ...
  const request_class = async (d1, d2, d3, d4, d5) => {
    ...
    const result3 = await axios.post(
      'https://' + process.env.REACT_APP_BACKEND_URL + 'balance/fetch/getbyid/balance',
      {
    ...
    if (result3.data.result.balance >= d5) {
      const result = await axios.post(
        'https://' + process.env.REACT_APP_BACKEND_URL + 'student/requestclass',
        {
          teacher_id: d1,
          democlass: false,
          starttime: d2,
      ...
      const result6 = await axios.post(
        'https://' + process.env.REACT_APP_BACKEND_URL + 'balance/update/balance',
        {
          user_id: result2.data.result.user_id,
  ...
  const acceptreq = async (id) => {
    ...
    const result = await axios.post(
      'https://' + process.env.REACT_APP_BACKEND_URL + 'teacher/class/response',
      {
        classid: id.id,
        response: 1,
      },
  ...
  useEffect(async () => {
    ...
    const result = await axios.post(
      'https://' + process.env.REACT_APP_BACKEND_URL + 'student/classroom/classes',
